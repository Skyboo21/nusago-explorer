...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-foreground flex items-center gap-3">
            <i data-lucide="plus-circle" class="h-8 w-8 text-primary"></i> Tambah Kuliner Baru
        </h1>
        <p class="text-muted-foreground mt-2">Isi detail destinasi kuliner untuk ditambahkan ke aplikasi</p>
    </div>
        ...
        <form action="{{ route('admin.kuliner.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <input type="text" name="nama_kuliner" value="{{ old('nama_kuliner') }}" 
                           placeholder="Cth: Sate Ayam Madiun Asli"
                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all @error('nama_kuliner') border-red-300 @enderror" required>
                ...
                <div>
                    ...
                    <input type="text" name="daerah" value="{{ old('daerah') }}" 
                           placeholder="Cth: Madiun, Jawa Timur"
                ...
                <div>
                    ...
                    <input type="number" name="harga_estimasi" value="{{ old('harga_estimasi') }}" 
                           placeholder="0"
                           class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <select name="wisata_id" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all bg-white">
                        ...
                        @foreach(\App\Models\Wisata::all() as $wisata)
                            <option value="{{ $wisata->id }}" {{ old('wisata_id') == $wisata->id ? 'selected' : '' }}>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="deskripsi_kuliner" rows="4" 
                              placeholder="Ceritakan keunikan rasa, sejarah, atau ciri khas kuliner ini..."
                              class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all">{{ old('deskripsi_kuliner') }}</textarea>
                </div>

                <!-- Rating & Jam Operasional -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Rating Awal (0-5)</label>
            ...
            <div class="mt-8 pt-6 border-t border-border flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-3 rounded-xl bg-primary text-white font-medium hover:bg-primary/90 shadow-sm transition-all flex items-center gap-2">
                    <i data-lucide="save" class="h-4 w-4"></i> Simpan Data
                </button>
            </div>
        </form>
    </div>
